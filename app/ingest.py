...
def emails_in_text(text):
    """Plain regex backstop so a clearly-written address is never missed."""
    seen, out = [], []
...
def ingest_post(text=None, url=None):
    ...
    to_list = [e for e in (data.get("recipient_emails") or []) if e and e.strip()]
    if not to_list:
        to_list = emails_in_text(text)        # catch an address the model missed
    to = _join(to_list)
