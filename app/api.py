...
from . import gmail_client, ingest, finder, productjobs_ingester
...
@app.get("/agent")
def agent_state():
    return finder.get_state()


@app.put("/agent")
def agent_update(body: AgentIn):
    return finder.set_state(enabled=body.enabled, search_queries=body.search_queries)


@app.post("/finder/run")
def finder_run():
    # manual "Find now": always searches; auto-sends only if the agent is enabled
    return finder.run_finder(manual=True)
...
scheduler.add_job(finder.maybe_scan, "interval", minutes=20, id="finder_scan", replace_existing=True)
